...
writes data/strata_packages/<site>/:
  ...
  strata.qml       QGIS style: Value Map on "stratum" (four strata +
                   unassigned) + a fill per stratum
  (strata_draft.gpkg / strata.gpkg are written by labelling/strata_rule.py;
  they replaced the empty strata.geojson template, which could not carry
  a field.)
...
from labelling.strata_style import COLOURS, strata_qml  # noqa: E402,F401
...
def build(site: str, cfg: dict) -> dict:
    ...
    write_geojson(os.path.join(out, "frame.geojson"),
                  ...
                  crs, "frame")
    stale = os.path.join(out, "strata.geojson")           # superseded by strata.gpkg
