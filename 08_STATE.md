...
| `LABELLING_GUIDE.md` | The item 21 hand-labelling protocol (~~v1.0, decided 2026-09-24~~ ~~v1.1~~ ~~v1.2~~ ~~v1.3~~ v1.4, 2026-09-25). **Read before labelling any tile.** |
...
|---|---|---|---|
| `master` | ~~`71f31fa`~~ ~~`a98b529`~~ `738c53b` | Parts 1–3; Part 8 items 47 and 70; `applicability-gating` merged; **`architecture-pivot-signoff` merged 2026-09-25** | The stable line. Default branch on GitHub. ~~**Does not yet contain the signed-off item 21 / Decisions 11-13-14 work**~~ Contains the signed-off item 21 / Decisions 11-13-14 work since `738c53b` |
| `architecture-pivot-signoff` | `0060afb` | Merges of `band-mapping-verification` (`4bd0d85`) and `unmixing-ceiling-investigation` (`32d066d`, 2026-09-23), C44 fix, endmember stability, and all 2026-09-24/25 decision records | ~~**Active working branch.** Ahead of `master`; not yet merged to it~~ **Merged to `master` at `738c53b`, 2026-09-25.** Branch retained |
| `item-21-phase-a` | *(see `git log`)* | Item 21 Phase A — everything that needs no training labels | **Active working branch**, cut from `master` at `738c53b` |
...
| **Producers** *(recorded 2026-09-24)* | Vegetation, water, impervious_total: spectral regression (item 21, 2026-09-23). snow_ice: spectral + low temporal variance. solar: spectral-signature detector (ground-mounted only, `excluded` per AOI from a global solar dataset — amended 2026-09-25). mixed_water_vegetation: spectral + GMW/GLWD sub-typing (mangrove producer = GMW extent directly, 2026-09-25). All run before the remainder. The single-unmixing-solve spec is struck as superseded. A "layered" stage-1 unmixing proposal was withdrawn, never decided |
| **Folded, flags only** | Sports fields/golf/parks/farmland → vegetation; sand/salt flats/rock/dry lakebeds/dirt tracks/landfills/quarries → bare; ~~docks → built~~ *(struck 2026-09-25: piers/quays are sealed decks → `paved` by the top-surface rule; `man_made=pier`/`quay` is a context flag only)* |
...
**Labelling guide:** [`LABELLING_GUIDE.md`](LABELLING_GUIDE.md), ~~v1.0~~ ~~v1.1~~ ~~v1.2~~ ~~v1.3~~ v1.4 since 2026-09-25 (v1.0 decided
